...
public class DVDAddView {
    public static void show() {
        ...
        successCount.put("producer", DVDDao.addProducer(producer)); //db에 producer 객체 넣기, 호출후에 id 변경

        Publisher publisher = Publisher.builder().publisherName(publisherName).build();
        successCount.put("publisher", DVDDao.addPublisher(publisher)); //db에 publisher 넣기
